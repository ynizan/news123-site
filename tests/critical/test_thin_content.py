...
def test_sitemap_has_proper_priorities(output_dir):
    ...
    for url, actual_priority in matches[:15]:  # Sample first 15 for speed
        ...
        # URL format: https://permitindex.com/{state}/{city}/apply-for-a-business-license/
        # or: https://permitindex.com/{state}/apply-for-a-business-license/
        url_parts = url.rstrip('/').split('/')

        # Need at least: https, '', permitindex.com, state, permit
        if len(url_parts) < 5:
            continue

        # Extract components
        if len(url_parts) == 5:
            # State-level: https://permitindex.com/{state}/{permit}/
            state_slug = url_parts[3]
            city_slug = None
        else:
            # City-level: https://permitindex.com/{state}/{city}/{permit}/
            state_slug = url_parts[3]
...
def test_noindex_matches_sitemap_priority(output_dir):
    ...
    for html_file in Path(output_dir).rglob("*/apply-for-a-business-license/index.html"):
        ...
        full_url = f"https://permitindex.com{url_path}"
...
def test_robots_txt_exists_with_sitemap(output_dir):
    ...
    assert 'https://permitindex.com/sitemap.xml' in robots_content, (
